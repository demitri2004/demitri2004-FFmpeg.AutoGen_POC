...
internal class Program
{
    ...
    //private static void Main(string[] args)
    //{
    //    Console.WriteLine("FFmpeg Video Stabilization");
    ...
    //    string inputVideoPath = "../../../../TestFiles/7131_cpcp_video_09CF56CD.mp4";
    //    string outputVideoPath = "stabilized_output.mp4";

    //    // Validate input file exists
    //    if (!System.IO.File.Exists(inputVideoPath))
    //    {
    //        Console.WriteLine($"Input file not found: {inputVideoPath}");
    //        return;
    //    }
    ...
    //    {
    //        // Stabilize the video
    //        Console.WriteLine("Starting video stabilization...");
    //        VideoStabilizer.StabilizeVideo(inputVideoPath, outputVideoPath);

    //        Console.WriteLine($"Video stabilization completed successfully. Output: {outputVideoPath}");
    //    }
    //    catch (Exception ex)
    //    {
    ...
    private static void ConfigureHWDecoder(out AVHWDeviceType HWtype)
    {
        ...
        // Automatically default to no hardware acceleration
        Console.WriteLine("Hardware acceleration for decoding is disabled by default.");
        Console.WriteLine("If you want to enable it, modify the code to select an available decoder.");

        // Below logic is retained for reference, but will not be executed unless explicitly re-enabled.
        ...
        {
            Console.WriteLine("Your system has no hardware decoders available.");
        }
        else
        {
            Console.WriteLine("Hardware decoders detected, but using software decoding by default.");
        }
    }
    ...
    //{
    //    // decode all frames from url, please not it might local resorce, e.g. string url = "../../7131_videoTmp_8.mp4";
    //    //"C:\Development\FFmpeg.AutoGen-master\TestFiles\7131_videoTmp_8.mp4"


    //    //string filePath = "../../../../TestFiles/7131_cpcp_video_09CF56CD.mp4";
    ...
    //    var url = "../../../../TestFiles/7131_cpcp_video_09CF56CD.mp4";
    ...
    private static unsafe void DecodeAllFramesToImages(AVHWDeviceType HWDevice)
    {
        var url = "../../../../UploadedVideo/7131_videoTmp_10.mp4";
        using var vsd = new VideoStreamDecoder(url, HWDevice);

        Console.WriteLine($"Codec name: {vsd.CodecName}");

        // Calculate frame rate
        double fps = vsd.FrameRate;
        Console.WriteLine($"Frame rate: {fps} FPS");

        // Calculate frame interval for 2 FPS
        var frameInterval = (int)(fps / 2);
        var frameNumber = 0;
        var savedFrameNumber = 1; // Start numbering from 1
        ...
        while (vsd.TryDecodeNextFrame(out var frame))
        {
            ...
            {
                var convertedFrame = vfc.Convert(frame);
                WriteFrame(convertedFrame, savedFrameNumber++);
                Console.WriteLine($"Saved frame: {savedFrameNumber - 1}");
            }
    ...
    {
        var frameFiles = Directory.GetFiles("../../../../Output", "frame_*.jpg").OrderBy(x => x).ToArray();
        using var fistFrameImage = ReadFrame(frameFiles.First());

        var outputFileName = "../../../../Output/out.h264";
        var fps = 25;
    ...
    {
        // Use zero-padded numbering with "frame_001", "frame_002", etc.
        string fileName = $"../../../../Output/frame_{frameNumber:D3}.jpg";
        ...
        bitmap.Encode(stream, SKEncodedImageFormat.Jpeg, 50);
    }
